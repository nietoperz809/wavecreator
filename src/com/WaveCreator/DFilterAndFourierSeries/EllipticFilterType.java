...
public abstract class EllipticFilterType extends PoleFilterType
{
    ...
    final double[] zf1 = new double[100];
    ...
    void setupElliptic(int a)
    {
        ...
        // xi = 1/k
        double xi = (Math.exp(dFilterFrame.auxBars[a + 1].getValue() / 1000.) - 1) * 5 + 1;
        ...
        for (i = 1; i <= n / 2; i++)
        {
            double u = (2 * i - ni) * K / n;
            double sn = calcSn(u);
            sn *= 2 * DFilterFrame.pi / K;
            f[i] = zeros[i - 1] = 1 / sn;
            //System.out.println("zero " + i + " " + zeros[i-1]);
        }
        zeros[n / 2] = 1e30;
        double fb = 1 / (2 * DFilterFrame.pi);
        nin = n % 2;
        n2 = n / 2;
